...
class GraspFashionHuanqiuSpider(scrapy.Spider):
	...
	def parse_detail(self, response):
		...
		pub_time = response.xpath("//div[@class='metadata-info']/p[@class='time']/text()").extract_first().split()[0]
		source = response.xpath(
			"//div[@class='metadata-info']/p[1]/span[@class='source']/span/a/text()").extract_first()
		content = ''.join(response.xpath("//div[@class='l-con clear']").extract())
		content_img = response.xpath("//article/section/p/i[@class='pic-con']/img/@src").extract()
		if content_img:
			content_img_list = list()
			for index, value in enumerate(content_img):
				img_name = title + str(index)
				if value.startswith('//'):
					res = upload_file.send_file('https:' + value, img_name, self.headers)
				else:
